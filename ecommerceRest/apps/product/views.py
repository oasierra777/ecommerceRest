...
class ListProductsView(APIView):
    ...
    def get(self, request, format=None):
        ...
        if not(sortBy == 'date_created' or sortBy == 'price' or sortBy == 'solid'or sortBy == 'name'):
            sortBy = 'date_created'
...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        ## Si categoryID es = 0, filtrar todas las categorías
        if category_id == 0:
            ...
            category = Category.objects.get(id=category_id)
            if category.parent:
                # Si la categoría tiene padre filtrar solo por la categoría y no el padre tambien
